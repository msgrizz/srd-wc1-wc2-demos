 ...
 *   Changes:
 *     - Initial version, now implements full command set.
 *       - Although several commands always report SUCCESS and actually
 *         do nothing.
 *     - Does not yet implement database/persistance (that is next step).
 * ********************************************************************************
 *
 **/
...
namespace OfficeAutomationServer
{
    class Program
    {
        ...
        public static bool HandleInputCommand(string strin, ClientConnectionThread client, bool loggedon,
            ClientConnectionThread.PermissionLevels permissions, NetworkStream ns)
        {
            bool handled = true;
            try
            {
                switch (strin.ToUpper())
                {
                    case "SHUTDOWN":
                        if (loggedon)
                        {
                            if (permissions == ClientConnectionThread.PermissionLevels.AdminUser)
                            {
                                client.SendToClient(ns, "SUCCESS SHUTDOWN");
                                m_quit = true;
                            }
                            else
                            {
                                client.SendToClient(ns, "FAILED SHUTDOWN " + ClientConnectionThread.ErrorCodes.InsufficientPermissions + " SHUTDOWN command requires that you have administrator permissions.");

                            }
                        }
                        else
                        {
                            client.SendToClient(ns, "FAILED SHUTDOWN " + ClientConnectionThread.ErrorCodes.UserNotLoggedOn + " SHUTDOWN command requires that you LOGON first.");
                        }
                        break;
                    case "VERSION":
                    case "GETVERSION":
