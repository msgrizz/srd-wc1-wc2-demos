...
using System.IO; // used for simulated phone audio
 ...
 *   Changes:
 *     - Recompiling to use latest PLTLabsAPI2 - the only changes are that
 *       it is using a different BladeRunner deckard event to receive the
 *       mic mute status, and it no longer confuses mobile calls with pc calls.
 *
 * Version 1.6.0.2:
 * Date: 27th Aug 2014
 * Compatible/tested with Hub / Spokes SDK version(s): 3.0.50718.1966
 * Changed by: Lewis Collins
 *   Changes:
 *     - Added warning dialog to make sure deckard.xml is placed in the EXE's
 *       working folder.
 *
 *  Version 1.6.0.1:
 * Date: 30th July 2014
 ...
 * Changed by: Lewis Collins
 *   Changes:
 *     - Recompiled against updated PLT Labs API (FF1A event changed to
 *       FF0D). Also included updated deckard.xml here.
 *
 * Version 1.6.0.0:
 * Date: 29th July 2014
 * Compatible/tested with Hub / Spokes SDK version(s): 3.0.50718.1966
...
{
    public partial class Form1 : Form, DebugLogger, PLTLabsCallbackHandler
    {
        ...
        delegate void UpdateHeadTrackingCallback();
        #endregion
        ...
        // Wearable Concept 1 API
        PLTLabsAPI2 m_pltlabsapi = null;
        PLTConnection m_pltlabsconnection = null;
        ...
        private int m_lastheading;
        private int m_lastpitch;
        private int m_lastroll;
        private bool m_callringing = false;
        private bool m_callinprogress = false;
        private PLTProximityType m_lastnearfar = PLTProximityType.Unknown;
        private Interop.Plantronics.DeviceBatteryLevel m_batterylevel;
        private Interop.Plantronics.COMDevice m_device;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!File.Exists("deckard.xml"))
            {
                MessageBox.Show("Warning: deckard.xml not found in EZ Demo's working folder! Head tracking will not be available!\r\n\r\nTo resolve this please ensure a copy of deckard.xml is placed in the EZ Demo EXE working folder, the run it again.", "EZ Demo Head Tracking Warning", MessageBoxButtons.OK, MessageBoxIcon.Hand);
            }
        ...
        private void DoPutOn(EventArgs e)
        {
        ...
        private void DoMobileCallerId(MobileCallerIdArgs e)
        {
        ...
        private void DoOnMobileCall(OnMobileCallArgs e)
        {
        ...
        private void DoNotOnMobileCall()
        {
...
ROMEO
    ...
    Therefore thy kinsmen are no let to me."
        ...
        void spokes_OnCall(object sender, OnCallArgs e)
        {
            ...
            m_callringing = e.State == OnCallCallState.Ringing;
            m_callinprogress = e.State == OnCallCallState.OnCall;
        ...
        private void DoMuteChanged(MuteChangedArgs e)
        {
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (m_pltlabsconnection != null)
                {
                    m_pltlabsconnection = null;
                }
        ...
        {
            LogMessage(MethodInfo.GetCurrentMethod().Name, "device: "
                + pltDevice.m_device.DeviceAddress);
            m_pltlabsconnection = null;
            m_gyrocal = false;
            UpdateHeadTrackingGUI();
        }
        ...
        {
            LogMessage(MethodInfo.GetCurrentMethod().Name, "device: "
                + pltConnection.m_device.m_device.DeviceAddress);

            m_pltlabsconnection = pltConnection;

            // register for headtracking
            if (m_pltlabsconnection != null && pltConnection.m_device != null)
            {
                ...
                m_pltlabsapi.subscribe(PLTService.CALLERID_SVC, PLTMode.On_Change,
                    200);
                m_pltlabsapi.subscribe(PLTService.MUTESTATE_SVC, PLTMode.On_Change,
                    200);
            }
        }
        ...
        {
            if (m_pltlabsconnection == null) return;
            switch (pltInfo.m_serviceType)
            {
                case PLTService.MOTION_TRACKING_SVC:
                    PLTMotionTrackingData trackdata = (PLTMotionTrackingData)pltInfo.m_data;
                    if (m_gyrocal)
                    {
                        LogMessage(MethodInfo.GetCurrentMethod().Name, "head: "
                            + (int)trackdata.m_orientation[0] + " ptch: "
                            + (int)trackdata.m_orientation[1] + " rll: "
                            + (int)trackdata.m_orientation[2]);
                        m_lastheading = -(int)trackdata.m_orientation[0];
                        m_lastpitch = (int)trackdata.m_orientation[1];
                        m_lastroll = (int)trackdata.m_orientation[2];
                        UpdateHeadTrackingGUI();
                    }
                    break;
                case PLTService.SENSOR_CAL_STATE_SVC:
                    ...
                    if (!m_gyrocal)
                    {
                        LogMessage(MethodInfo.GetCurrentMethod().Name, "gyrocal?: "
                            + caldata.m_isgyrocal + " magnocal: "
                            + caldata.m_ismagnetometercal);
                    }
                    m_gyrocal = caldata.m_isgyrocal;
                    UpdateHeadTrackingGUI();
                    break;
                case PLTService.WEARING_STATE_SVC:
                    PLTWearingState wearstate = (PLTWearingState)pltInfo.m_data;
                    LogMessage(MethodInfo.GetCurrentMethod().Name, "Worn?: "
                        + wearstate.m_worn + " initial state: "
                        + wearstate.m_isInitialStateEvent);
                    if (wearstate.m_worn)
                    {
                        DoPutOn(new WearingStateArgs(wearstate.m_worn, 
                            wearstate.m_isInitialStateEvent));
                    }
                    else
                    {
                ...
                case PLTService.PROXIMITY_SVC:
                    PLTProximity proxstate = (PLTProximity)pltInfo.m_data;
                    LogMessage(MethodInfo.GetCurrentMethod().Name, "Prox?: "
                        + proxstate.m_proximity + " RSSI: "
                        + proxstate.m_rssistrength);
                    if (m_lastnearfar != proxstate.m_proximity)
                    {
                        m_lastnearfar = proxstate.m_proximity;
                        if (proxstate.m_proximity == PLTProximityType.Near)
                        {
                            DoNear();
                        }
                        else
                ...
                case PLTService.CALLSTATE_SVC:
                    ...
                    LogMessage(MethodInfo.GetCurrentMethod().Name, "Call state: "
                        + callinfo.m_callstate);

                    if (callinfo.m_callsource == "Mobile")
                    {
                        if (callinfo.m_callstate == PLTCallState.Idle)
                        {
                            DoNotOnMobileCall();
                        }
                        else
                        {
                            MobileCallState mobstate;
                            switch (callinfo.m_callstate)
                            {
                                case PLTCallState.Idle:
                                    mobstate = MobileCallState.Idle;
                                    break;
                                case PLTCallState.OnCall:
                                    mobstate = MobileCallState.OnCall;
                                    break;
                                case PLTCallState.Ringing:
                                    mobstate = MobileCallState.Ringing;
                                    break;
                                default:
                                    mobstate = MobileCallState.Idle;
                                    break;
                            ...
                            DoOnMobileCall(new OnMobileCallArgs(callinfo.m_incoming, mobstate));
                        }
                    }
                    break;
                case PLTService.CALLERID_SVC:
                    PLTCallerId callerid = (PLTCallerId)pltInfo.m_data;
                    LogMessage(MethodInfo.GetCurrentMethod().Name, "Mobile Caller id: "
                        + callerid.m_callerid);

                    DoMobileCallerId(new MobileCallerIdArgs(callerid.m_callerid));
                    break;
                case PLTService.MUTESTATE_SVC:
                    PLTMuteState mutestate = (PLTMuteState)pltInfo.m_data;
                    LogMessage(MethodInfo.GetCurrentMethod().Name, "MUTE: "
                        + mutestate.m_muted);

                    DoMuteChanged(new MuteChangedArgs(mutestate.m_muted));
                    break;
            }
        }

        private void UpdateHeadTrackingGUI()
        {
            if (m_pltlabsconnection == null) return;
            try
            {
                if (headingTrackBar.InvokeRequired)
                {
                    UpdateHeadTrackingCallback d = new UpdateHeadTrackingCallback(UpdateHeadTrackingGUI);
                    this.Invoke(d, new object[] { });
                }
                else
                { 
                    headingTrackBar.Value = m_lastheading;
                    PitchTrackBar.Value = m_lastpitch;
                    RollTrackBar.Value = m_lastroll;

                    // demo answer/end call with head gestures
                    if (m_lastroll < -30 && m_callringing)
                    {
                        m_spokes.AnswerCall(m_callId);
                        m_callringing = false;
                    }
                    if (m_lastroll > 30 && m_callinprogress)
                    {
                        m_spokes.EndCall(m_callId);
                        m_callinprogress = false;
                    }

                    zero_angles_btn.Enabled = m_gyrocal;
                }
            }
            catch
            {
        ...
        private void zero_angles_btn_Click(object sender, EventArgs e)
        {
            if (m_pltlabsconnection != null)
            {
        ...
        private void applyheadtrackingrate_btn_Click(object sender, EventArgs e)
        {
            // this code doesn't work
            // seems to result in a double subcription

            //int newrate = 200;
            //try
            //{
            //    newrate = Convert.ToInt32(
            //        headtrack_rate_textbox.Text);

            //    if (newrate < 0)
            //    {
            //        LogMessage(MethodInfo.GetCurrentMethod().Name, "I didn't like your rate: "
            //            + headtrack_rate_textbox.Text + ", resetting to default: 200");
            //        headtrack_rate_textbox.Text = "200";
            //        newrate = 200;
            //    }
            //}
            //catch
            ...
            //    m_pltlabsapi.subscribe(PLTService.MOTION_TRACKING_SVC, PLTMode.Periodic,
            //        newrate);
            //}
        }
    }
