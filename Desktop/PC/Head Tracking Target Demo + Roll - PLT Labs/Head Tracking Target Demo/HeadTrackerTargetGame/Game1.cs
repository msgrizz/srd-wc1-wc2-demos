 ...
 * Changes:
 *   - Updated to add option to reverse pitch and roll with R key!
 *      > address known issue with PC headtracking API - pitch and roll sometimes are reversed!
 *   - Also that rolling head will enlarge the cross hairs
 *
 * Version 1.0.0.1:
...
namespace HeadTrackerTargetGame
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game, PLTLabsCallbackHandler
    {
        ...
        private float m_scalefactor = 1.0f; // test use Roll angle to scale crosshair
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Draw(SpriteTexture, pos, null, Color.White, 0f, m_origin, m_scalefactor, SpriteEffects.None, 0f );

            if (!m_calibrated)
            {
                spriteBatch.DrawString(font, "Awaiting calibration (place headset on table)", new Vector2(20, 100), Color.White);
            }
            if (m_autoputoncalibratetimer.Enabled)
            {
                spriteBatch.DrawString(font, "Headset put on, about to calibrate (2 seconds)...", new Vector2(20, 5), Color.White);
            }

            if (m_calibrated)
            {
                spriteBatch.DrawString(font, "Heading: "+(int)m_headingangle+
                    "\r\nPitch: " + (int)m_pitchangle +
                    "\r\nRoll: " + (int)m_rollangle, new Vector2(20, 25), Color.White);
            }
