...
#ifdef TARGET_OS_MAC
@interface PLTContextServer () <SRWebSocketDelegate, NSAlertDelegate>
#else
...
#ifdef TARGET_OS_MAC
@property(nonatomic,strong) NSAlert *reconnectAlert;
#else
...
#ifdef TARGET_OS_MAC
@synthesize reconnectAlert = _reconnectAlert;
#else
...
{
    BOOL messageWasSent = NO;
	NSString *title = @"Context Server Exception";
    
    // check that WebSocket is open
    if (self.state < PLT_CONTEXT_SERVER_OPEN) {
		
		NSString *message = @"WebSocket is not open.";
		
#ifdef TARGET_OS_MAC
		NSAlert *alert = [NSAlert alertWithMessageText:title defaultButton:@"OK" alternateButton:nil otherButton:nil informativeTextWithFormat:message, nil];
		[alert runModal];
#else
    ...
    // check that Server authentication is complete
		...
		NSString *message = @"Server requires authentication with username & password.";
...
- (void)showReconnectAlert
{
	...
	NSString *title = @"Connection Lost";
	NSString *message = @"Please wait while the context server connection is re-established...";
#ifdef TARGET_OS_MAC
	if(!self.reconnectAlert) {
		
		self.reconnectAlert = [NSAlert alertWithMessageText:title defaultButton:@"Cancel" alternateButton:nil otherButton:nil informativeTextWithFormat:message, nil];
		
		NSProgressIndicator *indicator = [[NSProgressIndicator alloc] initWithFrame:NSZeroRect];
		[indicator setStyle:NSProgressIndicatorSpinningStyle];
		[self.reconnectAlert setAccessoryView:indicator];
		[indicator startAnimation:self];
		
		NSInteger result = [self.reconnectAlert runModal];
		if (result == NSAlertDefaultReturn) {
			// "Cancel" was clicked
			[self reconnectAlertDidCancel];
		}
    }
#else
	if(!self.reconnectAlertView) {
        self.reconnectAlertView = [[UIAlertView alloc] initWithTitle:title
...
#ifdef TARGET_OS_MAC
	[self.reconnectAlert.window close];
