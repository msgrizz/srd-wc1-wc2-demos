...
- (BRQuaternion)quaternionFromServiceData:(NSData *)serviceData;
...
{
	if (!self.device) {
		NSArray *devices = [BRDevice availableDevices];
		if ([devices count]) {
	...
	if ([event isKindOfClass:[BRWearingStateChangedEvent class]]) {
		...
		switch (serviceID) {
			case BRDefinedValue_SubscribeToServicesCommand_ServiceID_ServiceID_Orientation: {
				BRQuaternion quaternion = [self quaternionFromServiceData:serviceData];
				BREulerAngles eulerAngles = BREulerAnglesFromQuaternion(quaternion);	
...
//				[self.rollIndicator setDoubleValue:eulerAngles.z];
			...
			case BRDefinedValue_SubscribeToServicesCommand_ServiceID_ServiceID_Taps: {
				...
				if (count) self.tapsLabel.text = [NSString stringWithFormat:@"%d in %d", count, direction];
