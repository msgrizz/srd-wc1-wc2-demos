...
NSString *const		TKey_SettingResultSuccessMuxCaseBlock =	@"setting_result_success_mux_case_block";
NSString *const		TKey_EventMuxCaseBlock =				@"event_mux_case_block";
NSString *const		TKey_ExceptionCaseBlock =				@"exception_mux_case_block";
...
- (void)generateCode
{
	...
	NSMutableString *tval_settingResultSuccessMuxCaseBlock = [NSMutableString string];
	NSMutableString *tval_eventMuxCaseBlock = [NSMutableString string];
	NSMutableString *tval_exceptionMuxCaseBlock = [NSMutableString string];
	...
	for (DeckardMessage *message in allMessages) {
		...
		// pre-message template completion is done. if it's in incoming message, add the apropriate bits to the incoming message mux
		if (message.type == DeckardMessageTypeSettingResult) {
			[tval_settingResultSuccessMuxCaseBlock appendFormat:@"\t\t\t\tcase %@:\n\t\t\t\t\tclass = [BR%@ class];\n\t\t\t\t\tbreak;\n", tval_IdentifierConstantName, tval_MessageName];
		}
		else if (message.type == DeckardMessageTypeEvent) {
			[tval_eventMuxCaseBlock appendFormat:@"\t\t\t\tcase %@:\n\t\t\t\t\tclass = [BR%@ class];\n\t\t\t\t\tbreak;\n", tval_IdentifierConstantName, tval_MessageName];
		}
		else if (message.type == DeckardMessageTypeException) {
			[tval_exceptionMuxCaseBlock appendFormat:@"\t\t\t\tcase %@:\n\t\t\t\t\tclass = [BR%@ class];\n\t\t\t\t\tbreak;\n", tval_IdentifierConstantName, tval_MessageName];
		}
	}
	
	// write incoming mux file
	
	NSFileManager *fm = [NSFileManager defaultManager];
	NSString *pwd = [fm currentDirectoryPath];
	NSMutableString *muxHBody = [NSMutableString stringWithContentsOfFile:[NSString stringWithFormat:@"%@/Templates/BRIncomingMessageMux.h.template", pwd]
																		 encoding:NSUTF8StringEncoding error:nil];
	NSMutableString *muxMBody = [NSMutableString stringWithContentsOfFile:[NSString stringWithFormat:@"%@/Templates/BRIncomingMessageMux.m.template", pwd]
																		 encoding:NSUTF8StringEncoding error:nil];
	...
	[self populateTemplate:muxHBody withString:tval_DeckardVersion forKey:TKey_DeckardVersion];
	[self populateTemplate:muxMBody withString:tval_DeckardVersion forKey:TKey_DeckardVersion];
	[self populateTemplate:typeImportsHBody withString:tval_DeckardVersion forKey:TKey_DeckardVersion];
	
	[self populateTemplate:muxHBody withString:tval_MDY forKey:TKey_MDY];
	[self populateTemplate:muxMBody withString:tval_MDY forKey:TKey_MDY];
	[self populateTemplate:typeImportsHBody withString:tval_MDY forKey:TKey_MDY];
	
	[self populateTemplate:muxHBody withString:tval_YYYY forKey:TKey_YYYY];
	[self populateTemplate:muxMBody withString:tval_YYYY forKey:TKey_YYYY];
	[self populateTemplate:typeImportsHBody withString:tval_YYYY forKey:TKey_YYYY];
	
	[self populateTemplate:muxMBody withString:tval_settingResultSuccessMuxCaseBlock forKey:TKey_SettingResultSuccessMuxCaseBlock];
	[self populateTemplate:muxMBody withString:tval_eventMuxCaseBlock forKey:TKey_EventMuxCaseBlock];
	[self populateTemplate:muxMBody withString:tval_exceptionMuxCaseBlock forKey:TKey_ExceptionCaseBlock];
	...
	NSString *muxHFilePath = [dir stringByAppendingPathComponent:@"BRIncomingMessageMux.h"];
	NSString *muxMFilePath = [dir stringByAppendingPathComponent:@"BRIncomingMessageMux.m"];
	NSString *typeImportsHFilePath = [dir stringByAppendingPathComponent:@"BRTypeImports.h"];
	[muxHBody writeToFile:muxHFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
	[muxMBody writeToFile:muxMFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	if ([elementName isEqualToString:@"section"]) {
		...
		// special cases -- catch c/obj-c keywords
