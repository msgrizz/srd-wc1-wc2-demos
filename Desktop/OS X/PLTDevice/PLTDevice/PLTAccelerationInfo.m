...
@property(nonatomic,assign,readwrite)	PLTAcceleration	acceleration;

@end


@implementation PLTAccelerationInfo
...
- (void)parseServiceData
{
	...
	self.acceleration = (PLTAcceleration) {
...
{
	return [NSString stringWithFormat:@"<PLTAccelerationInfo: %p> requestType=%lu, timestamp=%@, calibration=%@, acceleration=%@",
			self, self.requestType, self.timestamp, self.calibration, NSStringFromAcceleration(self.acceleration)];
}
