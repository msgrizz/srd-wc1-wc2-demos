...
{
	return [NSString stringWithFormat:@"{ %f, %f, %f }", angularVelocity.x, angularVelocity.y, angularVelocity.z];
}


@interface PLTAngularVelocityInfo()

@property(nonatomic,assign,readwrite)	PLTAngularVelocity	angularVelocity;

@end


@implementation PLTAngularVelocityInfo
...
- (void)parseServiceData
{
	...
	self.angularVelocity = (PLTAngularVelocity) {
...
{
	return [NSString stringWithFormat:@"<PLTAngularVelocityInfo: %p> requestType=%lu, timestamp=%@, calibration=%@, angularVelocity=%@",
			self, self.requestType, self.timestamp, self.calibration, NSStringFromAngularVelocity(self.angularVelocity)];
}
