...
import com.plantronics.DX650ScreenLock.MainApp;
import com.plantronics.DX650ScreenLock.PlantronicsXEventMessage;
...
public class BluetoothReceiver extends BroadcastReceiver {
	...
	PlantronicsXEventListener listener;
	...
	public void onReceive(Context context, final Intent intent) {
		...
		PlantronicsXEventMessage m = null;

		if (ACTION_ACL_CONNECTED.equals(action)) {
			m = new PlantronicsXEventMessage(
					PlantronicsXEventMessage.HEADSET_CONNECTED_EVENT);
			m.addProperty("bdaddr", bdAddr);

			Log.e(LOG_TAG, "message: " + m.toString());

			MainApp.headsetACLConnected();

		} else if (ACTION_ACL_DISCONNECTED.equals(action)) {
			m = new PlantronicsXEventMessage(
					PlantronicsXEventMessage.HEADSET_DISCONNECTED_EVENT);
			m.addProperty("bdaddr", bdAddr);

			Log.e(LOG_TAG, "message: " + m.toString());

			MainApp.headsetACLDisconnected();

		} else if (ACTION_VENDOR_SPECIFIC_HEADSET_EVENT.equals(action)) {
			m = generateMessageFromEvent(intent);
			if (m != null) {
			...
			m = new PlantronicsXEventMessage(
					PlantronicsXEventMessage.CALL_STATUS_CHANGED_EVENT);
			m.addProperty("currentHeadsetAudioState", AUDIOSTATE_LOOKUP
					.get(intent.getExtras().getInt(EXTRA_STATE)));
			m.addProperty(
					"previousHeadsetAudioState",
					AUDIOSTATE_LOOKUP.get(intent.getExtras().getInt(
							EXTRA_PREVIOUS_STATE)));

			Log.e(LOG_TAG, "message: " + m.toString());
		} else {
			Log.e(LOG_TAG, "Action came in and was not processed: " + action);
		}

		if (m != null) {
			if (m.getEventType().equals("DON")) {
				Log.e(LOG_TAG, "don event");
				MainApp.headsetDonned();
			} else if (m.getEventType().equals("DOFF")) {
				Log.e(LOG_TAG, "doff event");
				MainApp.headsetDoffed();
			}
		}
	}
	...
	private static int PROFILE_ARRAY[] = { A2DP, AVRCP, HFP, HSP, NEO, CASIRA,
			TURBO_BASE, DONGLE };
	...
	private PlantronicsXEventMessage generateMessageFromEvent(Intent intent) {
		...
		Log.d(LOG_TAG, "Event from Plantronics headset = " + eventName);
		...
		Log.e(LOG_TAG, "Unknown event " + eventName);
		return null;
	}
}
