...
@interface SecurityHelper () <PLTDeviceBRDevicePassthroughDelegate, NSURLConnectionDelegate>
...
@property (nonatomic,assign)	id <SecurityHelperEnrollDelegate>	enrollDelegate;
@property (nonatomic,assign)	id <SecurityHelperSignDelegate>		signDelegate;
...
+ (SecurityHelper *)sharedHelper
{
	...
	if (!instance) instance = [[SecurityHelper alloc] init];
	return instance;
}
...
{
	NSLog(@"enroll: %@", delegate);
	
	self.enrollDelegate = delegate;
	
	if (SENSOR_DEVICE) {
		self.enrollDelegate = delegate;
		CONNECTED_DEVICE.passthroughDelegate = self;
		...
		[self.enrollDelegate securityHelper:self didEncounterErrorEnrolling:(NSError *)@"Error: Headset not connected"];
	}
}
...
- (void)sign:(id <SecurityHelperSignDelegate>)delegate;
{
	...
	if (SENSOR_DEVICE) {
		...
		[self.signDelegate securityHelper:self didEncounterErrorAuthenticating:(NSError *)@"Not connected"];
	}
}
	...
	switch (self.state) {
		...
		case PLTFIDOThingStateSigning: {
			...
			if (status != 0x0090) { // 0x9000 with byte swap
				...
				[self.signDelegate securityHelper:self didEncounterErrorAuthenticating:@"Bad status"];
			}
			else {
				NSLog(@"Good status.");
...
- (void)requestEnrollmentDataFromServer
{
	...
	NSString *urlString = [NSString stringWithFormat:@"%@/enrollData.js?userName=%@&password=%@", self.serverAddress, self.serverUsername, self.serverPassword];
...
- (void)parseEnrollmentResponceFromServer:(NSDictionary *)responce
{
	...
	if (responce[@"attestationCertificate"]) {
		...
		[self.enrollDelegate securityHelperDidEnroll:self];
	}
	else {
		NSLog(@"*** Error enrolling device: %@ ***", @"sometgubg");
		//self.textField.stringValue = @"Error enrolling.";
		[self.enrollDelegate securityHelper:self didEncounterErrorEnrolling:(NSError *)@"Parse error"];
	}
...
- (void)requestSignDataFromServer
{
	...
	[self.signDelegate securityHelper:self didUpdateSigningState:SigningStateRetrievingEnrollments];
	
	// ARTIFICIAL DELAY
	float delayInSeconds = ARTIFICIAL_DELAY;
	dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, delayInSeconds * NSEC_PER_SEC);
	dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
		NSString *urlString = [NSString stringWithFormat:@"%@/signData.js?userName=%@&password=%@", self.serverAddress, self.serverUsername, self.serverPassword];
		NSLog(@"urlString: %@", urlString);
...
- (void)parseSignDataFromServer:(NSArray *)signData
{
	...
	if ([signData count]) {
		...
		[self.signDelegate securityHelperDidFailAuthenticate:self];
		self.state = PLTFIDOThingStateIdle;
	}
}
...
- (void)sendSignCommandToDevice:(NSDictionary *)signRequest
{
	...
	[self.signDelegate securityHelper:self didUpdateSigningState:SigningStateSigningOnDevice];
...
- (void)sendSignatureToServer:(NSData *)signedAPDU
{
	...
	[self.signDelegate securityHelper:self didUpdateSigningState:SigningStateVerifyingSignatureWithServer];
...
- (void)parseSignatureResponceFromServer:(NSDictionary *)response
{
	...
	if ([response[@"result"] isEqualToString:@"success"]) {
		...
		[self.signDelegate securityHelperDidAuthenticate:self];
	}
	else {
...
- (void)BRDevice:(BRDevice *)device didRaiseCommandException:(BRException *)exception
{
	...
	switch (self.state) {
		...
		case PLTFIDOThingStateEnrollingDevice:
		case PLTFIDOThingStateSendingEnrollment:
			[self.enrollDelegate securityHelper:self didEncounterErrorEnrolling:@"Exception"];
			break;
		case PLTFIDOThingStateRequestingSignData:
		case PLTFIDOThingStateSigning:
		case PLTFIDOThingStateSendingSignature:
			[self.signDelegate securityHelper:self didEncounterErrorAuthenticating:@"Exception"];
			break;
