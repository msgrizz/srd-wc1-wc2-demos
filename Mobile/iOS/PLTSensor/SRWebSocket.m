...
{    
    ...
    if (_secure) {
        ...
        #if DEBUG || IGNORE_CERTIFICATE_ERRORS
        [SSLOptions setValue:[NSNumber numberWithBool:NO] forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
...
- (void)send:(id)data;
{
    ...
    dispatch_async(_workQueue, ^{
        if ([data isKindOfClass:[NSString class]]) {
            ...
            // BEWARE! - binary frame opcode may cause failure when sending UTF8 encoded JSON as NSData payload
...
{
    //SRFastLog(@"Received message: delegate = %@, msg =%@", self.delegate, message);
    dispatch_async(_callbackQueue, ^{
...
- (void)_pumpWriting;
{
    ...
    if (_closeWhenFinishedWriting && 
        ...
        /*********************************
         Added by Morgan 11-26-12.
         I don't fully understand how this class works, but I've observed that if a connection is lost due to backgrounding->foregrounding an app,
         the disconnect fails and the client (delegate) is never notified. So I added a new code to SRStatusCode.
         *********************************/
//        else {
//            dispatch_async(_callbackQueue, ^{
//                if ([self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
//                    [self.delegate webSocket:self didCloseWithCode:SRStatusCodeDisconnectFailure reason:_closeReason wasClean:NO];
//                }
//            });
//        }
