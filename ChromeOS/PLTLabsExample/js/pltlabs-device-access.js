...
//using the Google Bluetooth APIs 
...
var hostNegotiate = function(){
  var packet = PLTLabsMessageHelper.createHostNegotiateMessage();
  ...
  switch(messageType){
    case PLTLabsMessageHelper.PROTOCOL_VERSION_TYPE:
     ...
     case PLTLabsMessageHelper.METADATA_TYPE:
        ...
        //messageHelper.parseMetadata(data, function(deviceMetadata){
...
PLTLabsMessageHelper.createHostNegotiateMessage = function(){
  var options = new Object();
  options.messageType = this.PROTOCOL_VERSION_TYPE;
  var address = new ArrayBuffer(this.BR_ADDRESS_SIZE);
...
PLTLabsMessageHelper.createEnableTestInterfaceCommand = function(options){
  if(!options.enabled){
    console.log('createEnableTestInterfaceCommand: options requires boolean field: enabled');
    return null;
  }
