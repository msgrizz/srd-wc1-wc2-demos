...
plt.msg = (function (){
  ...
  my.PASSWORD_CHANGED_EVENT = 0x0F16;
...
//END EVENTS  //COMMANDS
  ...
  my.SET_PASSWORD_COMMAND = 0x0F16;
  //Section HAL Interface (0x11nn)
  ...
  supportedCommands.push(my.SET_PASSWORD_COMMAND);
...
//END COMMANDS  //SETTINGS
  ...
  my.PASSWORD_SETTING = 0x0F16;
  my.PROTECTED_STATE_SETTING = 0x0F18;
  //Section HAL Interface (0x11nn)
 ...
 var supportedSettings = [];
  ...
  supportedSettings.push(my.PASSWORD_SETTING);
  supportedSettings.push(my.PROTECTED_STATE_SETTING);
...
//END SETTINGS  //EXCEPTIONS
  ...
  my.FEATURE_IS_LOCKED_EXCEPTION = 0x0F14;
...
//END EXCEPTIONS
  ...
  case my.SET_PASSWORD_COMMAND:
    var bufferSize = 0;
      var passwordLength = msg.password.length + 2;
      bufferSize += (passwordLength * 2);
      var data = new ArrayBuffer(bufferSize);
      var data_view = new Uint8Array(data);
      var index = 0;
      //store the array length before adding the array
      data_view[index++] = (0xFF00 & msg.password.length) >> 8;
      data_view[index++] = (0x00FF & msg.password.length);
      for(i = 0; i < msg.password.length; i++){
        var charCode = msg.password.charCodeAt(i);
        data_view[index++] = (0xFF00 & charCode) >>  8;
        data_view[index++] = (0x00FF & charCode);
      }
      msg.messageData = data;
  ...
  var parseGetSettingResult = function(message){
    ...
    switch(payload.messageId){
       ...
       case my.PASSWORD_SETTING:
         ...
         index += 2;
         bounds = (index + (2 * arrayLength));
         payload.password = parseString(index, data_view, bounds);
         index = bounds;

         break;    
       
       case my.PROTECTED_STATE_SETTING:
         var index = 2;
         var bounds = 0;
         var arrayLength = 0;
         payload.name = 'Protected state';
         payload.state = byteToBool(data_view[index]);
         index++;
  ...
  var parseEventResult = function(message){
    ...
    switch(payload.messageId){
       ...
       case my.VOICE_RECOGNITION_TEST_EVENT_ENABLE_DISABLE:
         ...
         payload.voiceRecogntionEventEnable = byteToBool(data_view[index]);
         index++;
       ...
       case my.VOICE_RECOGNITION_TEST_EVENT:
         ...
         payload.voiceRecognitonId = parseShort(index, data_view);
         index += 2;
       ...
       case my.SET_FEATURE_LOCK_EVENT:
         ...
         payload.lock = byteToBool(data_view[index]);
         index++;
         arrayLength = parseShort(index, data_view);
         index += 2;
         bounds = (index + (2 * arrayLength));
